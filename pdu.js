...
export const parse = pdu => {
  ...
  cursor = (smscSize * 2 + 2);
  var smscNum = deSwapNibbles(pdu.slice(4, cursor));
  ...
  var senderNum;
  if (senderType === '91') {
    senderNum = deSwapNibbles(encodedSender);
  ...
  var encoding = detectEncoding(dataCodingScheme);

  var timestamp = deSwapNibbles(pdu.slice(cursor, cursor + 14));
  ...
  if (encoding === '16bit')
    var text = decode16Bit(pdu.slice(cursor), dataLength);
  else if (encoding === '7bit')
    var text = decode7Bit(pdu.slice(cursor), dataLength);
...
export const detectEncoding = function (dataCodingScheme) {
...
export const decode16Bit = function (data, length) {
...
export const decode7Bit = function (code, count) {
...
export const encode7Bit = function (ascii) {
...
export const generate = function (message) {
  var pdu = '00';
  ...
  var receiver = swapNibbles(message.receiver);
...
export const encode16Bit = function (text) {
  var out = '';
...
export const swapNibbles = function (nibbles) {
  var out = '';
  for (var i = 0; i < nibbles.length; i = i + 2) {
...
export const parseStatusReport = function (pdu) {
  //Cursor points to the last octet we've read.
  var cursor = 0;
  ...
  var smscNum = deSwapNibbles(pdu.slice(cursor, (smscSize * 2) + 2));
  cursor = (smscSize * 2 + 2);
  ...
  var sender = deSwapNibbles(pdu.slice(cursor, cursor + senderSize));
...
export function randomHexa(size) {
  var text = "";

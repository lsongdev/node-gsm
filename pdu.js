...
PDU.parse = function(pdu) {
  ...
  cursor = (smscSize*2+2);
  var smscNum  = PDU.deSwapNibbles(pdu.slice(4, cursor));
  ...
  var senderNum;
  if (senderType === '91') {
      senderNum = PDU.deSwapNibbles(encodedSender);
  ...
  var encoding = PDU.detectEncoding(dataCodingScheme);

  var timestamp = PDU.deSwapNibbles(pdu.slice(cursor, cursor+14));
  ...
  if(encoding === '16bit')
      var text = PDU.decode16Bit(pdu.slice(cursor), dataLength);
  else if(encoding === '7bit')
      var text = PDU.decode7Bit(pdu.slice(cursor), dataLength);
...
PDU.detectEncoding = function(dataCodingScheme) {
...
PDU.decode16Bit = function(data, length) {
...
PDU.deSwapNibbles = function(nibbles) {
  var out = '';
...
PDU.decode7Bit = function(code, count) {
...
PDU.generate = function(message) {
  ...
  var receiver = PDU.swapNibbles(message.receiver);
  ...
  for(var i=0; i< parts; i++) {
      ...
      if(message.encoding === '16bit') {
          user_data = PDU.encode16Bit(text);
...
PDU.swapNibbles = function(nibbles) {
  var out = '';
  for(var i = 0; i< nibbles.length; i=i+2) {
...
PDU.parseStatusReport = function(pdu) {
  //Cursor points to the last octet we've read.
  var cursor = 0;
  ...
  var smscNum  = PDU.deSwapNibbles(pdu.slice(cursor, (smscSize*2)+2));
  cursor = (smscSize*2+2);
  ...
  var sender = PDU.deSwapNibbles(pdu.slice(cursor, cursor+senderSize));
